## Abstract
본 프로젝트는 simple version of autonomous driving simulation engine을 구현하기 위함이다.
...
├── main_viewer.py             # BEV, Sensor Projection 시각화 모듈
├── main_planning_control.py   # 메인 실행 파일 (Planner & Control 통합)
│
├── src/
│   ├── world/                 # Perception: 시나리오 및 도로 환경 구축 
│   │   ├── world.py           # 월드 포인트 및 기본 맵 생성
│   │   └── osm_converter.py   # OSM(OpenStreetMap) → Polyline 변환
│   │
│   ├── sensors/               # Perception: 센서 데이터 처리 및 인지 
│   │   ├── bev.py             # BEV(Bird's Eye View) 변환 및 클리핑
│   │   ├── camera_pinhole.py  # 카메라 핀홀 모델 구현
│   │   ├── camera_extrinsic.py# 차량 내 센서 장착 위치 설정
│   │   └── perception.py      # 인지 결과 통합 및 환경 정보 센서 데이터 변환 
│   │
│   ├── planning/              # Planner: 행동 결정 및 경로 생성 
│   │   ├── planner.py         # 상황별 판단 및 경로 설정 모듈 
│   │   └── path.py            # 기준 경로(Reference Path) 생성
│   │
│   ├── dynamics/              # Control & Dynamics: 제어 및 차량 거동 
│   │   ├── vehicle.py         # 차량 상태 및 Bicycle Model 파라미터 정의
│   │   └── control.py         # MPC/Pure Pursuit/PID 제어기 구현 
│   │
│   └── common/                # 공통 유틸리티
│       ├── types.py           # 공통 데이터 구조 정의 (Closed-loop 데이터 흐름)
│       └── math_utils.py      # 행렬 연산, 좌표 변환 등 보조 수학 함수 
│
└── assets/                    # 리소스 및 설정
    ├── maps/                  # 지도 데이터 (OSM 등)
    │   └── map.osm
    └── configs/               # 시뮬레이션 파라미터 설정 파일
```
...
- Ground Truth 관점: 실차에서는 “가볍고 검증된 모델로 inference만” 하고, 학습/튜닝은 시뮬·오프라인 파이프라인에서 데이터로 간접 기여.

## 시뮬레이션을 돌리기 위한 4개 모듈 (기능 · 입력 · 출력 · 확장)
...
- 확장 방향(Extension)
    ...
    - (실제) NVIDIA는 Omniverse/센서 RTX API로 고충실도 센서 시뮬레이션을 강조
